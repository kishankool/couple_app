...
export default function App() {
  ...
  const [who, setWho] = useState(() => localStorage.getItem('ka_who') || 'Kishan')
  ...
  // Ask who you are on first visit
  useEffect(() => {
    if (!localStorage.getItem('ka_who')) setShowWhoModal(true)
  }, [])
      ...
      <WhoContext.Provider value={{ who, setWho: selectWho }}>
        <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
          ...
          <nav style={styles.nav}>
            {NAV.map(n => (
              <button key={n.path} style={{...styles.navItem, ...(location.pathname === n.path ? styles.navActive : {})}} onClick={() => navigate(n.path)}>
                <span style={{ fontSize: '1.25rem', lineHeight: 1 }}>{n.icon}</span>
                <span style={{ fontSize: '0.58rem', textTransform: 'uppercase', letterSpacing: '0.5px', color: location.pathname === n.path ? 'var(--mauve-deep)' : 'var(--text-light)', fontWeight: location.pathname === n.path ? 700 : 400 }}>{n.label}</span>
              </button>
            ))}
          </nav>
          ...
          {/* Who are you modal */}
          <Modal open={showWhoModal} onClose={() => who && setShowWhoModal(false)} title="Who are you? 💕">
            <p style={{ fontSize: '0.88rem', color: 'var(--text-light)', marginBottom: 16, textAlign: 'center' }}>
              Tell us who's using the app right now so we can personalise your experience!
            </p>
