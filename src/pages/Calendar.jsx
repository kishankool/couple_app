...
export default function Calendar() {
  ...
  const save = async () => {
    ...
    // Validate that the date parts form a real calendar date
    const [fy, fm, fd] = fDate.split('-').map(Number)
    if (!fy || !fm || !fd || isNaN(new Date(`${fDate}T12:00:00`).getTime())) {
      return showToast('Invalid date — please pick again 📅')
    }
    const maxDay = getDaysInMonth(fy, fm - 1)
    if (fd > maxDay) {
      return showToast(`${fm}/${fy} only has ${maxDay} days 📅`)
    }
